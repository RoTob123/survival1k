...
-- generate terrain, which is random right now.
...
plr_x = 64 -- player's x (duh)
plr_y = 64 -- player's y (DUUUHH)
...
-- basic controls
if btn(3) and plr_y <= 315 then
	plr_y += 1
elseif btn(2) and plr_y >= 0 then
	plr_y -= 1
end
if btn(0) and plr_x >= 0 then
	plr_x -= 1
...
if btnp(5) and atk_dur == 0 then
	...
	for i=1,#enemies do -- check for victims
		if enemies[i] then -- idk??? it crashed a lot without this
			...
			if abs(enemies[i][2]-plr_x) < 10 and abs(enemies[i][3]-plr_y) < 10 and enemies[i][5] == 0 then
				enemies[i][4] -= atk -- deal damage
				enemies[i][5] = 10 -- iframes
...
if (hp==0) print("\as8bagfedc")goto dead -- if hp = 0, go to game over screen
